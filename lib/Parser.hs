...
parseVariableSection (Pos (f, r, c) VarKeyword : rest) = do
    ...
    case rest' of
        (Pos _ Semicolon : b@(Pos _ BeginKeyword) : rest'') ->
            return (Pos (f, r, c) . VariableSection . (:[]) <$> names, b:rest'')
        (Pos _ Semicolon : i@(Pos _ (Identifier _)) : rest'') -> do
            (section, rest''') <- parseVariableSection $ (Pos (f, 0, 0) VarKeyword):i:rest''
            return $ (section >>= \(Pos _ (VariableSection s)) ->
                Pos (f, r, c) . VariableSection . (:s) <$> names, rest''')
        (Pos _ Semicolon : Pos pos token :rest'') -> do
            tellError pos $
                "Expected either the begin keyword or an identifier, found " ++ show token
...
parseVariableDefinition :: [Pos Token] ->
    Writer [String] (Maybe ([Pos Name], Pos Type), [Pos Token])
parseVariableDefinition (Pos pos (Identifier name) :
                         Pos _ Comma : i@(Pos _ (Identifier _)) : rest) = do
...
parseVariableDefinition (Pos _ (Identifier _) : Pos _ Comma : Pos pos token : rest) = do
    ...
    return (Nothing, Pos pos token : rest)
parseVariableDefinition (Pos namePos (Identifier name) : Pos _ Colon :
                         Pos typePos IntegerType : rest) =
    return (Just ([Pos namePos $ Name name], Pos typePos AST.IntegerType), rest)
parseVariableDefinition (Pos namePos (Identifier name) : Pos _ Colon :
                         Pos typePos BooleanType : rest) =
    return (Just ([Pos namePos $ Name name], Pos typePos AST.BooleanType), rest)
...
parseOperatorSection (Pos pos BeginKeyword : rest) =
    (fmap . fmap) (Pos pos) $ parseOperator rest >>= uncurry parseOperatorSectionRest
parseOperatorSection (Pos pos token : _) = do
    tellError pos $
        "Expected the begin keyword at the beginning of the operator section. Actual: "
        ++ show token
    return Nothing
parseOperatorSection [] = return Nothing

parseOperatorSectionRest :: Maybe (Pos Operator) -> [Pos Token] ->
    Writer [String] (Maybe OperatorSection)
parseOperatorSectionRest op rest = case rest of
    ...
    (Pos _ EndKeyword : Pos pos token : _) -> do
        tellError pos $
            "Expected a dot after the end of the operator section. Actual: " ++ show token
        return $ OperatorSection . (:[]) <$> op
...
parseOutputOperator (Pos _ WritelnKeyword : Pos pos token : rest) = do
    tellError pos $
        "Expected an opening parenthesis for the writeln function call. Actual: " ++ show token
    return (Nothing, Pos pos token : rest)
parseOutputOperator tokens = return (Nothing, tokens)

parseOutputOperatorRest :: [Pos Expression] -> [Pos Token] ->
    Writer [String] (Maybe Operator, [Pos Token])
...
parseOutputOperatorRest args (Pos pos token : rest) = do
    when (not $ null args) $
        tellError pos $ "Expected a closing parenthesis. Actual: " ++ show token
    return (Nothing, rest)
...
parseArgumentList tokens = do
    ...
    if isJust expr then case rest of
        ...
        (Pos pos token : _) -> do
            tellError pos $ concat [
                    "Expected either a comma or a closing parenthesis ",
...
parseSwitchVariant :: [Pos Token] ->
    Writer [String] (Maybe (Pos AST.Constant, Pos Operator), [Pos Token])
...
parseSwitchVariantsRest :: [Pos Token] ->
    Writer [String] (Maybe [(Pos AST.Constant, Pos Operator)], [Pos Token])
            ...
            case operator1 of
                ...
                (Just op1, Pos _ ElseKeyword : rest'') -> do
                    ...
                    case operator2 of
                        n@(Nothing, _) -> return n
                        (Just op2, rest''') ->
                            return (Just . Pos pos $ IfOperator expr op1 op2, rest''')
    ...
    case parsed of
        ...
        (Just (Pos pos expr), Pos opPos t : ts) | isJust $ toRelationOp t -> do
            ...
            return $ case parsed' of
                ...
                (Just expr', rest) ->
                    (fmap (Pos pos) $
                        pure RelationExpression <*> (Pos opPos <$> toRelationOp t)
                                                <*> pure (Pos pos expr)
                                                <*> pure expr', rest)
...
parseSumExpression tokens = do
    ...
    case parsed of
        (Nothing, rest) -> return (Nothing, rest)
        (Just (Pos pos prod), rest) ->
            parseSumLoop (Pos pos . SimpleSumExpression $ Pos pos prod) rest
    where
        parseSumLoop (Pos accPos acc) (Pos opPos t : ts) | isJust $ toAdditionOp t = do
            ...
            case parsed of
                (Nothing, rest) -> return (Nothing, rest)
                (Just expr, rest) -> let
                    newAcc = Pos accPos $ AdditionExpression (Pos opPos . fromJust $ toAdditionOp t)
                                                             (Pos accPos acc) expr
                    in parseSumLoop newAcc rest
        parseSumLoop acc tokens' = return (Just acc, tokens')
        
parseProductExpression :: [Pos Token] ->
    Writer [String] (Maybe (Pos ProductExpression), [Pos Token])
parseProductExpression tokens = do
    parsed <- parseBasicExpression tokens
    case parsed of
        (Nothing, rest) -> return (Nothing, rest)
        (Just (Pos pos basic), rest) ->
            parseProductLoop (Pos pos . SimpleProductExpression $ Pos pos basic) rest
    where
        parseProductLoop (Pos accPos acc) (Pos opPos t : ts) | isJust $ toMultiplicationOp t = do
            ...
            case parsed of
                ...
                (Just expr, rest) -> let
                    newAcc = Pos accPos $
                        MultiplicationExpression (Pos opPos . fromJust $ toMultiplicationOp t)
                        (Pos accPos acc) expr
...
parseBasicExpression (Pos pos token : rest) = case token of
    ...
    IntegerConstant i ->
        return (Just . Pos pos . ConstantExpression . Pos pos $ AST.IntegerConstant i, rest)
    BooleanConstant b ->
        return (Just . Pos pos . ConstantExpression . Pos pos $ AST.BooleanConstant b, rest)
    NotOperator -> do
    ...
    OpenParen -> do
        ...
        return $ case parsed of
            (Just expr, (Pos _ CloseParen : rest')) ->
                (Just . Pos pos $ ParenthesizedExpression expr, rest')
            (_, rest') -> (Nothing, rest')
    _ -> return (Nothing, rest)

parseConstant :: [Pos Token] -> Writer [String] (Maybe (Pos AST.Constant), [Pos Token])
parseConstant (Pos pos (IntegerConstant i) : rest) =
    return (Just . Pos pos $ AST.IntegerConstant i, rest)
parseConstant (Pos pos (BooleanConstant b) : rest) =
    return (Just . Pos pos $ AST.BooleanConstant b, rest)
